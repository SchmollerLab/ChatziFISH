...
for spotM_path in beyond_listdir_pos.spotM_paths:
    ...
    for filename in os.listdir(spotM_path):
        ...
        elif filename.find('4_AllPos_spotfit_MOTH_data')!=-1:
            spotfit_df_moth = pd.read_csv(csv_path, index_col=idx)
            spotfit_df_moth_li.append(spotfit_df_moth)
        elif filename.find('4_AllPos_spotfit_BUD_data')!=-1:
            spotfit_df_bud = pd.read_csv(csv_path, index_col=idx_bud)
            spotfit_df_bud_li.append(spotfit_df_bud)
        elif filename.find('4_AllPos_spotfit_TOT_data')!=-1:
            spotfit_df_tot = pd.read_csv(csv_path, index_col=idx)
            spotfit_df_tot_li.append(spotfit_df_tot)
...
if spotfit_df_moth_li:
    spotfit_df_moth = pd.concat(spotfit_df_moth_li, keys=keys,
                               names=names)
if spotfit_df_bud_li:
    spotfit_df_bud = pd.concat(spotfit_df_bud_li, keys=keys,
                               names=names)
if spotfit_df_tot_li:
    spotfit_df_tot = pd.concat(spotfit_df_tot_li, keys=keys,
                               names=names)
...
spotfit_df_moth_filename = f'{name}_4_AllExp_spotfit_MOTH_data.csv'
spotfit_df_bud_filename = f'{name}_4_AllExp_spotfit_BUD_data.csv'
spotfit_df_tot_filename = f'{name}_4_AllExp_spotfit_TOT_data.csv'
...
if spotfit_df_moth_li:
    spotfit_df_moth.to_csv(f'{main_data_path}/{spotfit_df_moth_filename}',
                           encoding='utf-8-sig')
if spotfit_df_bud_li:
    spotfit_df_bud.to_csv(f'{main_data_path}/{spotfit_df_bud_filename}',
                           encoding='utf-8-sig')
if spotfit_df_tot_li:
    spotfit_df_tot.to_csv(f'{main_data_path}/{spotfit_df_tot_filename}',
                           encoding='utf-8-sig')
