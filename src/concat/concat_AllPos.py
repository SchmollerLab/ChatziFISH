...
for selected_path in tqdm(selected_paths, ncols=100, unit='experiment'):
    ...
    if not rs.skip:
        AllPos_summary_df = core.spotMAX_concat_pos(
            TIFFs_path, vNUM=vNUM, run_num=run_num, do_save=True
        )

        print('')
        print(f'Loading all dataframes from {selected_path}...')
        # Iterate position folders and concatenate dataframes
        AllPos_summary_df.load_df_from_allpos(
            vNUM=vNUM, run_num=run_num, verbose=True
        )
        ...
        if AllPos_summary_df.post_process_df_li:
            (post_process_df_moth, post_process_df_bud,
            post_process_df_tot) = AllPos_summary_df.generate_bud_moth_tot_dfs(
                                AllPos_summary_df.post_process_df_li
            )
        ...
        if AllPos_summary_df.post_process_df_li:
            AllPos_summary_df.save_AllPos_df(
                post_process_df_moth, '5_AllPos_post_process_MOTH_data.csv'
            )
            AllPos_summary_df.save_AllPos_df(
                post_process_df_bud, '5_AllPos_post_process_BUD_data.csv'
            )
            AllPos_summary_df.save_AllPos_df(
                post_process_df_tot, '5_AllPos_post_process_TOT_data.csv'
            )
