...
temp_path = os.path.join(script_dirpath, 'temp')
...
initialdir = '/'
last_dir_path = os.path.join(temp_path, '.last_dir.txt')
if os.path.exists(last_dir_path):
    with open(last_dir_path) as txt:
        initialdir = txt.read()
...
selected_path = prompts.folder_dialog(title=
    ...
    'a specific Position_n folder', initialdir=initialdir)

if not selected_path:
    exit('Execution aborted.')

with open(last_dir_path, 'w') as txt:
    txt.write(selected_path)
...
for exp_idx, main_path in enumerate(main_paths):
    ...
    for pos_idx, path in enumerate(paths[ps:pe]):
        ...
        if spotMAX_inputs.show:
            ...
            detection_mode = spotMAX_inputs.detection_mode
            ...
            filter_by_ref_ch_mask = spotMAX_inputs.filter_by_ref_ch_mask

            revert_gop = spotMAX_inputs.revert_gop
        ...
        (zyx_resolution,
        zyx_resolution_pxl,
        airy_radius) = core.calc_resolution_limited_vol(
            wavelen, NA, yx_resolution_multiplier,
        ...
        for frame_i in range(*frange):
            ...
            spots_lab_frames_dict[f'frame_{frame_i}'] = np.zeros(
                V_spots.shape, dtype=np.uint16
            )
            spots_lab = spots_lab_frames_dict[f'frame_{frame_i}']
            ...
            if segm_npy_frame is not None:
                ...
                if local_or_global_thresh == 'Local':
                    ...
                    if load_PC.is_first_call:
                        ...
                        if load_PC.PC_df is not None:
                            load_PC.load_single_PC(TIFFs_path, user_ch_name,
                                                   sigma)
                    (local_max_coords,
                    V_spots_sharp, spots_props) = core.spot_detection_local(
                    ...
                    (local_max_coords,
                    V_spots_sharp, spots_props) = core.spot_detection_global(
                        rp_segm_3D, V_spots,
                        local_maxima_thresh_func,
                        zyx_resolution_pxl, ref_mask,
                        segm_npy_3D, wavelen, NA, zyx_vox_dim,
                        yx_resolution_multiplier,
            ...
            if inspect:
                if not detection_mode == 'peak_local_max':
                    apps.imshow_tk(
                       V_spots, additional_imgs=[spots_lab],
                       dots_coords=local_max_coords, x_idx=2
                    )
            ...
            if len(local_max_coords) == 0:
                ...
                orig_summary.generate_summary_df(IDs, 0, segm_npy_3D,
                                    ...
                                    V_spots.shape, rp_segm_3D, is_segm_3D, None,
                ...
                if do_gaussian_fit:
                    spotfit_summary.generate_summary_df(
                        IDs, 0, segm_npy_3D, zyx_vox_dim, timestamp, 
                        finterval, frame_i, ref_mask, None, df_norm_ref_ch, 
                        cca_df, V_spots.shape, rp_segm_3D, is_segm_3D, None,
                        ref_chQUANT_data=orig_summary,
                        predict_cell_cycle=is_ref_single_obj)
                orig_summary.df_zyx = []
                continue
            ...
            if spots_props is not None:
                spots_props = core.filter_spots_props(
                    spots_props, ellips_test_idx
                )
            ...
            while num_nucl < num_peaks and num_nucl != 0:
                ...
                if spots_props is not None:
                    filtered_spot_props = {}
                    for key in list(spots_props.keys()):
                        filtered_spot_props[key] = df_gop_test[key].to_list()
                    spots_props = filtered_spot_props
            ...
            orig_summary.generate_summary_df(orig_df_MulIDs.IDs_unique,
                                            ...
                                            df_norm_ref_ch, cca_df,
                                            V_spots.shape, rp_segm_3D,
                                            is_segm_3D, orig_df_MulIDs.df_IDs,
                                            predict_cell_cycle=is_ref_single_obj)
            ellip_summary.generate_summary_df(ellips_df_MulIDs.IDs_unique,
                                        ellips_df_MulIDs.num_spots,
                                        segm_npy_3D, zyx_vox_dim,
