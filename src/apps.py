...
class auto_select_slice:
    ...
    def find_contours(
            self, label_img, cells_ids, group=False, concat=False,
            return_hull=False
        ):
        contours = []
        for id in cells_ids:
            ...
            cont, hierarchy = cv2.findContours(
                uint8_img,cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE
            )
            cnt = cont[0]
            if return_hull:
...
class inspect_effect_size_app:
    ...
    def run(self, load_ref_ch, V_spots, segm_npy_3D, IDs, local_max_coords,
        ...
        if not np.all(segm_npy_3D):
            segm_npy_2D = segm_npy_3D.max(axis=0)
            try:
                contours = auto_select_slice().find_contours(
                    segm_npy_2D, IDs, group=True
                )
            except IndexError:
                contours = []
        else:
