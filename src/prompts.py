...
class spotFIT_inputs:
    ...
    def prompt(self, df_inputs, is_segm_3D):
        ...
        # Filter by ref_ch
        df_row_i = 'Filter by segmenting ref. channel?'
        self.filter_by_ref_ch_mask = None
        if df_row_i in df_inputs.index:
            var = df_inputs.at[df_row_i, 'Values']
            filter_by_ref_ch_mask = self.isYes(var)
            var = df_inputs.at['Load a reference channel?', 'Values']
            load_ref_ch = self.isYes(var)
            self.filter_by_ref_ch_mask = filter_by_ref_ch_mask and load_ref_ch
        else:
            # Segmentation info ('2D' or '3D')
            idx += 1
            self.filter_by_ref_ch_mask_var = tk.IntVar()
            self.filter_by_ref_ch_mask_label_txt = df_row_i
            self.filter_by_ref_ch_mask_label = tk.Label(
                root, text=self.filter_by_ref_ch_mask_label_txt,
                font=(None, 10)
            )
            self.filter_by_ref_ch_mask_label.grid(row=idx, sticky=tk.E,
                                             pady=(10,0), padx=(10,0))
            self.filter_by_ref_ch_mask_yes_rb = ttk.Radiobutton(root,
                                        text='Yes',
                                        variable=self.filter_by_ref_ch_mask_var,
                                        value=1)
            self.filter_by_ref_ch_mask_yes_rb.grid(row=idx, padx=(4, 10),
                                              pady=(10,0), column=1)

            self.filter_by_ref_ch_mask_no_rb = ttk.Radiobutton(root,
                                        text='No',
                                        variable=self.filter_by_ref_ch_mask_var,
                                        value=0)
            self.filter_by_ref_ch_mask_no_rb.grid(row=idx, padx=(4, 10),
                                             pady=(10,0), column=2)
            self.filter_by_ref_ch_mask_var.set(1)
...
class spotMAX_inputs_widget:
    ...
    def run(self, title='Analysis inputs', channel_name='mNeon',
        ...
        # Segment reference channel radio button
        idx += 1
        self.segment_ref_var = tk.IntVar()
        self.segment_ref_label_txt = 'Filter by segmenting ref. channel?'
        self.segment_ref_label = tk.Label(
                                         root,
                                         text=self.segment_ref_label_txt,
                                         font=(None, 10))
        self.segment_ref_label.grid(row=idx, sticky=tk.E, pady=(10,0))
        self.segment_ref_yes_rb = ttk.Radiobutton(root,
                                            text='Yes',
                                            variable=self.segment_ref_var,
                                            value=0)
        self.segment_ref_yes_rb.grid(row=idx, padx=(4, 10), pady=(10,0),
                                     column=1, sticky=tk.W )
        self.segment_ref_no_rb = ttk.Radiobutton(root,
                                            text='No',
                                            variable=self.segment_ref_var,
                                            value=1)
        self.segment_ref_no_rb.grid(row=idx, padx=(4, 10), pady=(10,0),
                                    column=2, sticky=tk.W)
        self.segment_ref_var.set(1)
        ...
        # Detection methods
        idx += 1
        detection_methods = ['peak_local_max', 'threshold and label']
        self.detection_methods_label = tk.Label(root, text='Detection method:',
                                           font=(None, 10))
        self.detection_methods_label.grid(row=idx, sticky=tk.E, pady=(10,0))
        detection_method_var = tk.StringVar()
        detection_method_cb = ttk.Combobox(root, width=25, justify='center',
                                         textvariable=detection_method_var)
        detection_method_cb.option_add('*TCombobox*Listbox.Justify', 'center')
        detection_method_cb['values'] = detection_methods
        detection_method_cb.current(0)
        detection_method_cb.grid(row=idx, padx=(4, 10), pady=(10,0),
                                        column=1, columnspan=2)
        self.detection_method_cb = detection_method_cb
        self.detection_methods = detection_methods
        self.detection_method_var = detection_method_var
        ...
        # Segment reference channel radio button
        idx += 1
        self.reverse_gop_var = tk.IntVar()
        self.reverse_gop_label_txt = f'Invert the filtering of good peaks?'
        self.reverse_gop_label = tk.Label(
                                         root,
                                         text=self.reverse_gop_label_txt,
                                         font=(None, 10))
        self.reverse_gop_label.grid(row=idx, sticky=tk.E, pady=(10,0))
        self.reverse_gop_yes_rb = ttk.Radiobutton(root,
                                            text='Yes',
                                            variable=self.reverse_gop_var,
                                            value=0)
        self.reverse_gop_yes_rb.grid(row=idx, padx=(4, 10), pady=(10,0),
                                     column=1, sticky=tk.W )
        self.reverse_gop_no_rb = ttk.Radiobutton(root,
                                            text='No',
                                            variable=self.reverse_gop_var,
                                            value=1)
        self.reverse_gop_no_rb.grid(row=idx, padx=(4, 10), pady=(10,0),
                                    column=2, sticky=tk.W)
        self.reverse_gop_var.set(1)
    ...
    def set_last_status(self):
        if self.last_status_df is not None:
            ...
            try:
                # Filter by reference channel
                filter_by_ref_ch_mask = (self.last_status_df
                                     .at['Filter by segmenting ref. channel?', v])
                segment_ref_val = 0 if self.isYes(filter_by_ref_ch_mask) else 1
                self.filter_by_ref_ch_mask_var.set(segment_ref_val)
            except Exception as e:
                pass

            try:
                # Single object ref channel
            ...
            try:
                # Detection methods
                detection_method = (self.last_status_df
                                    .at['Detection method:', v])
                idx = self.detection_methods.index(detection_method)
                self.detection_method_cb.current(idx)
            ...
            try:
                # Filter by reference channel
                revert_gop = (self.last_status_df
                                     .at['Invert the filtering of good peaks?', v])
                revert_gop_val = 0 if self.isYes(revert_gop) else 1
                self.filter_by_ref_ch_mask_var.set(revert_gop_val)
            except Exception as e:
                pass
    ...
    def hide_ref_ch_inputs(self):
        if self.load_ref_ch_var.get() == 1:
            ...
            self.segment_ref_label.grid_remove()
            self.segment_ref_yes_rb.grid_remove()
            self.segment_ref_no_rb.grid_remove()
            ...
            self.segment_ref_label.grid()
            self.segment_ref_yes_rb.grid()
            self.segment_ref_no_rb.grid()
    ...
    def _quit(self, event=None):
        ...
        self.detection_mode = self.detection_method_var.get()
        ...
        self.filter_by_ref_ch_mask = self.segment_ref_var.get() == 0
        ...
        self.segm_info = self.segm_info_var.get()
        self.revert_gop = self.reverse_gop_var.get() == 0
        ...
        self.entry_txts = [
            ...
            self.gop_how, self.which_effsize, self.revert_gop,
            self.detection_mode, self.gop_limit_txt, self.load_ref_ch,
            self.filter_by_ref_ch, self.filter_by_ref_ch_mask, self.filter_z_bound,
        ...
        self.entry_labels.append(self.reverse_gop_label_txt)
        self.entry_labels.append(self.detection_methods_label.cget('text'))
        ...
        self.entry_labels.append(self.segment_ref_label_txt)
...
class multi_entry_messagebox:
    def __init__(
            self, title='Entry', entries_labels=('',),
            default_entries=('',), toplevel=True
        ):
        if toplevel:
            root = tk.Toplevel()
        else:
        ...
        row = 0
        self.entries = []
        for txt, input_txt in zip(entries_labels, default_entries):
            tk.Label(root, text=txt, font=(None, 10)).grid(row=row, padx=8)
            e = tk.Entry(root, justify='center')
            e.grid(row=row+1, padx=16, pady=4)
            e.insert(0, input_txt)
        ...
        tk.Button(
            root, command=self._quit, text='Ok!', width=10
            ).grid(row=row+1, pady=4)
        root.bind('<Return>', self._quit)
        root.protocol("WM_DELETE_WINDOW", self.on_closing)
        root.mainloop()
    ...
    def _quit(self, event=None):
        self.entries_txt = [entry.get() for entry in self.entries]
        self._root.quit()
        self._root.destroy()
...
if __name__ == '__main__':
    num_frames_prompt = multi_entry_messagebox(toplevel=False)
